...
現在は Chrome ウェブストア未公開のため、フォルダを直接読み込む方式でインストールします。

1. このリポジトリをダウンロード（`Code` → `Download ZIP`）して解凍する
2. Chrome で `chrome://extensions` を開く
3. 右上の「デベロッパー モード」をオンにする
4. 「パッケージ化されていない拡張機能を読み込む」をクリックし、解凍した `gtm-version-checker` フォルダを選択する

以上で完了です。GTMの管理画面でコンテナを開くと、自動的に動作を始めます。
